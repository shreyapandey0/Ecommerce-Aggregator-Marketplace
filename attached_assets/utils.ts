...
export function createTheme(theme: Record<string, any>) {
  ...
  if (theme.primary) {
    ...
    root.style.setProperty('--primary-foreground', toHslValues(theme.primary.foreground));
  }
  
  // Process secondary color
  if (theme.secondary) {
    root.style.setProperty('--secondary', toHslValues(theme.secondary.DEFAULT));
    root.style.setProperty('--secondary-foreground', toHslValues(theme.secondary.foreground));
  }
  
  // Process accent color
  if (theme.accent) {
    root.style.setProperty('--accent', toHslValues(theme.accent.DEFAULT));
    root.style.setProperty('--accent-foreground', toHslValues(theme.accent.foreground));
  }
  ...
  if (theme.card) {
    ...
    root.style.setProperty('--card-foreground', toHslValues(theme.card.foreground));
  }
...
function toHslValues(color: string): string {
  ...
  return color.startsWith('hsl') ? color.replace('hsl(', '').replace(')', '') : color;
}
